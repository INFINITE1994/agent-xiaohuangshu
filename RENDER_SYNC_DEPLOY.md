...
作者工作台支持拖拽或选择 **JPG、PNG、WebP**，会先在浏览器中自动缩放并压缩为 WebP，再插入 Markdown 图像链接。压缩后每张图片限制为 **2.5 MB**；二进制文件保存在 SQLPub 的 `agent_content_images` 表中，只有通过作者会话的请求可以上传。公开阅读、章节 PDF 和整本 PDF 通过 `GET /v1/images/:imageId` 读取图片，不暴露数据库凭据。

上传图片后，仍需点击章节的“保存并发布”，该图片链接才会进入面向所有访客的公开内容版本。图片库会显示历史图片并允许重复插入；已被公开正文或当前草稿引用的图片不能删除。未被引用的图片可在二次确认后删除。
